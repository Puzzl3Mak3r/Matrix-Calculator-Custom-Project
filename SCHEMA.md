...
**Core Requirement:** The codebase must explicitly demonstrate the four core pillars of Object-Oriented Programming: Abstraction, Encapsulation, Inheritance, and Polymorphism.
**Design Patterns:** The application must cleanly implement a minimum of three distinct software design patterns to achieve loose coupling and high cohesion.
**Architectural Expectation:** Code must be modular, maintainable, and easily upgradable. Classes must be isolated rather than compacted into 7–8 massive files to prevent long-term maintenance decay. The expected scope is approximately 10 to 30 classes.
**Timeline Constraint:** Final code delivery is firmed for the End of Week 13. It must pass a rigorous Week 14 panel interview where code quality and modifiability will be verified by live modifications.
...
### Clipboard Services & Formats (Commands)
* **Conversion Formats:** The system must convert any selected matrix's numerical state into distinct copy+paste formats: Direct (Raw numbers), LaTeX, and Formatted ASCII.
* **Formatting Syntax:** A $2 \times 2$ identity matrix must string-serialize precisely as:
  `\begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}`
* **System Clipboard Integration:** Users must be able to issue Copy and Paste commands that route to the OS clipboard, enabling seamless pasting into external editors.

---

## 3. System Architecture & Design Patterns

To secure the highest grading criteria and maintain loose coupling, the architecture is strictly split into domain logic, polymorphic operational classes, creational factories, and encapsulated command operations.
...
+-------+-------+
| MatrixOperations | <--- (Strategy Pattern Interface)
+-------+-------+
|
+-------> MatrixAddition
|
...
* **Implementation:** `MatrixOperations` acts as the strategy interface. Concrete math implementations execute the calculations independently.
...
#### `MatrixData` (Struct)
* **Description:** Encapsulates the raw dimensional properties and 2D array representation of a matrix.
* **Fields:**
  * `public int rows;` - The row count.
  * `public int cols;` - The column count.
...
#### `MatrixOperations` (Strategy Interface)
* **Method:**
...
1. Initialize the SplashKit drawing loop canvas framework.
